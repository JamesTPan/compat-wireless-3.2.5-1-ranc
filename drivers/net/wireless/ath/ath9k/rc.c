...
/**
 * fixed rate for coding packets usimg 11g rate table
 * Author:	James Tsunghsiao Pan (100062587)
 * purpose:	rate control of coding packets
 */
static bool coding_and_g(const struct ath_rate_table *rate_table, struct sk_buff *skb, u8 *rix)
{
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb->data;
	if(!ieee80211_is_coding(hdr->frame_control))
		return false;

	if(unlikely(rate_table != &ar5416_11g_ratetable)){
		printk(KERN_DEBUG "[ath9k] RC: Wrong rate table!\n");
		return false;
	}

	*rix = 8;

	if(ieee80211_is_coding_batchack(hdr->frame_control) || ieee80211_is_coding_dcrd(hdr->frame_control))
		*rix = (*rix >= 4)?(*rix-4) : 0; 
			 ...
			 struct ieee80211_tx_rate_control *txrc)
{
	...
	// fixed rate for coding packets
	if(!coding_and_g(rate_table, txrc->skb, &rix))
			  ...
			  struct sk_buff *skb)
{
	...
	// ADD CODING
	if (!priv_sta || !(ieee80211_is_data(fc) || ieee80211_is_coding(fc)))
		return;
