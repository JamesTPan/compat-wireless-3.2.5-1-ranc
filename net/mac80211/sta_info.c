...
static int __must_check __sta_info_destroy(struct sta_info *sta)
{
	...
	printk(KERN_DEBUG "[DEBUG] CALL: __sta_info_destroy()");
			  ...
			  unsigned long exp_time)
{
	struct ieee80211_local *local = sdata->local;
	struct sta_info *sta, *tmp;
	u8 dst[ETH_ALEN] = {0x1c, 0x4b, 0xd6, 0x8c, 0xc3, 0xbc};
	u8 rly[ETH_ALEN] = {0x44, 0x6d, 0x57, 0x47, 0x27, 0x6f};

	mutex_lock(&local->sta_mtx);
	list_for_each_entry_safe(sta, tmp, &local->sta_list, list)
		if (time_after(jiffies, sta->last_rx + exp_time)) {
			if(compare_ether_addr(sta->sta.addr, dst) == 0 || compare_ether_addr(sta->sta.addr, rly) == 0) continue;
